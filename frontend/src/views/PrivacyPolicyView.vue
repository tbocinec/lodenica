...
/**
 * Privacy policy page. Public read (also the URL Facebook Login requires);
 * admins get an "Upraviť" button that swaps the rendered body for a TipTap
 * WYSIWYG editor. Mirrors ReservationRulesView. PATCH is admin-gated.
 */
import { onMounted, ref } from 'vue';

import { privacyPolicyApi } from '@/api/privacy-policy.api';
...
async function load(): Promise<void> {
  ...
  error.value = null;
  try {
    const data = await privacyPolicyApi.get();
...
async function save(): Promise<void> {
  ...
  error.value = null;
  try {
    const data = await privacyPolicyApi.update(draft.value);
    content.value = data.content;
    updatedAt.value = data.updatedAt;
    editing.value = false;
  } catch (e) {
...
<template>
  <PageHeader
    ...
    subtitle="Aké osobné údaje systém spracúva a na aký účel."
  >
    <template #actions>
  ...
  <article
    ...
    v-html="content || '<p class=\'text-slate-400\'>Obsah zatiaľ nebol vyplnený.</p>'"
  />
