...
/**
 * Demo boats, reservations and a damage for local development (INST-001).
 * Runs only in the `local` environment or with SEED_DEMO_DATA=true, and
 * only into an empty inventory — a fresh client's production database
 * never gets sample kayaks.
 */
class DemoDataSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (Resource::query()->count() > 0) {
            $this->command?->info('Skipping demo data — DB already has resources.');
